    ...
    if (cap_supported(CAP_FIDO2)) {
        if (cmd == CTAPHID_CBOR) {
            ...
            switch (data[0]) {
                ...
                case CTAP_GET_INFO: wallet_ui_command("GET INFO"); break;
                default: wallet_ui_command("OTHER REQUEST"); break;
            }
#endif
...
void *cbor_thread(void *arg) {
    ...
    while (1) {
        ...
        if (cmd == CTAPHID_CBOR && cancel_button) {
            apdu.sw = 0x2d; // CTAP2_ERR_KEEPALIVE_CANCEL
            res_APDU_size = 0;
        }
        if (apdu.sw == 0) {
...
int cbor_process(uint8_t last_cmd, const uint8_t *data, size_t len) {
    ...
    // USB receive storage is reused by CANCEL while this worker is running.
