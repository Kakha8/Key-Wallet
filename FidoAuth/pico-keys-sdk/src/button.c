...
#ifdef ESP_PLATFORM
static bool approve_armed, approve_raw, approve_stable, cancel_raw;
static bool physical_cancel;
static uint32_t approve_changed, cancel_changed;
#endif
#endif
...
static bool picok_board_button_read(void) {
    int boot_state = gpio_get_level(GPIO_NUM_5);
    return boot_state == 1;
}
...
void button_wait_start(void) {
...
#ifdef ESP_PLATFORM
    // Physical confirmation is mandatory on this board, even with default PHY settings.
    ...
    // Preserve a host cancellation received before the worker requested presence.
#ifdef ESP_PLATFORM
    // Display the prompt; OLED failure is not user cancellation.
    wallet_ui_prompt();
#endif
    ...
    approve_armed = false;
    approve_raw = approve_stable = picok_board_button_read();
    cancel_raw = gpio_get_level(GPIO_NUM_6) == 1;
    approve_changed = cancel_changed = async_button_started;
#endif
...
void button_wait_poll(void) {
    ...
    if ((uint32_t)(now - approve_changed) >= 30) {
        approve_stable = approve_raw;
        if (!approve_stable) approve_armed = true;
    }
    pressed = approve_stable;
    bool cancel = gpio_get_level(GPIO_NUM_6) == 1;
    if (cancel != cancel_raw) {
        cancel_raw = cancel;
        cancel_changed = now;
    }
    if (cancel_raw && (uint32_t)(now - cancel_changed) >= 30) {
        if (!cancel_button) physical_cancel = true;
        cancel_button = true;
    }
    if (!approve_armed) pressed = false;
#endif
...
#ifdef ESP_PLATFORM
    // A pending cancellation wins over approval when both buttons are pressed.
    if (result == BUTTON_EV_PRESSED && cancel_raw) {
        physical_cancel = true;
        result = BUTTON_EV_CANCELLED;
    }
    wallet_ui_result(result == BUTTON_EV_PRESSED ? "APPROVED" :
                     result == BUTTON_EV_TIMEOUT ? "TIMED OUT" :
                     physical_cancel ? "GPIO SIX CANCEL" : "HOST CANCEL");
#endif
