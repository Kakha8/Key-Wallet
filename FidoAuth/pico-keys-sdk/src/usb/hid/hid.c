...
int driver_process_usb_packet_hid(uint16_t read) {
    ...
    if (read == HID_RPT_SIZE) {
        ...
        if (FRAME_TYPE(ctap_req) == TYPE_INIT && ctap_req->init.cmd == CTAPHID_CANCEL) {
            // CANCEL has no separate response. Let the worker finish the
            // active request once; never clear buffers it is still using.
            if (MSG_LEN(ctap_req) == 0 && is_busy() && last_cmd == CTAPHID_CBOR
                    && ctap_req->cid == last_req.cid) {
                printf("HOST CTAPHID_CANCEL\n");
                cancel_button = true;
            }
            return 0;
        }
