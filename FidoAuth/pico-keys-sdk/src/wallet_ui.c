...
static void line(unsigned page, const char *s) {
    unsigned x = 4;
    for (; *s && x + 5 < 128; ++s, x += 6)
        if (*s >= 'A' && *s <= 'Z') memcpy(frame + page * 128 + x, letters[*s-'A'], 5);
}
static bool draw(const char *a, const char *b, const char *c) {
    if (!oled) return false;
    memset(frame, 0, sizeof(frame));
    line(0, "KEY WALLET REV E"); line(2, a); line(4, b); line(6, c);
...
void wallet_ui_init(void) {
    ...
    draw("READY", "WAITING FOR REQUEST", "");
}
bool wallet_ui_prompt(void) {
    waiting = true;
    shown_command = atomic_load(&command);
    status = "OK OR CANCEL";
    return draw(shown_command, status, "PRESS AND RELEASE OK");
}
void wallet_ui_result(const char *message) {
    waiting = false;
    status = message;
    draw(shown_command, status, "");
    previous_buttons = -1;
}
...
void wallet_ui_task(void) {
    ...
    if (pins != previous_buttons) {
        static const char *levels[] = {
            "OK LOW CANCEL LOW", "OK HIGH CANCEL LOW",
            "OK LOW CANCEL HIGH", "OK HIGH CANCEL HIGH"
        };
        draw(shown_command, status, levels[pins]);
